...
async def create_session(session_data: SessionCreate, database: Session = Depends(get_db)) -> Session:
    ...
    db_session = Session(
        duration=session_data.duration,        sessionDate=session_data.sessionDate,        sessionId=session_data.sessionId,        sessionType=session_data.sessionType.value,        skillmatch_1_id=session_data.skillmatch_1        )
...
async def bulk_create_session(items: list[SessionCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_session = Session(
                duration=item_data.duration,                sessionDate=item_data.sessionDate,                sessionId=item_data.sessionId,                sessionType=item_data.sessionType.value,                skillmatch_1_id=item_data.skillmatch_1            )
            database.add(db_session)
...
async def create_skillmatch(skillmatch_data: SkillMatchCreate, database: Session = Depends(get_db)) -> SkillMatch:
    ...
    db_skillmatch = SkillMatch(
        createdDate=skillmatch_data.createdDate,        status=skillmatch_data.status.value,        matchId=skillmatch_data.matchId,        startDate=skillmatch_data.startDate,        user_2_id=skillmatch_data.user_2,        user_3_id=skillmatch_data.user_3        )
...
async def bulk_create_skillmatch(items: list[SkillMatchCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_skillmatch = SkillMatch(
                createdDate=item_data.createdDate,                status=item_data.status.value,                matchId=item_data.matchId,                startDate=item_data.startDate,                user_2_id=item_data.user_2,                user_3_id=item_data.user_3            )
            database.add(db_skillmatch)
            database.flush()  # Get ID without committing
...
async def create_skillrequest(skillrequest_data: SkillRequestCreate, database: Session = Depends(get_db)) -> SkillRequest:
    ...
    db_skillrequest = SkillRequest(
        status=skillrequest_data.status.value,        requestId=skillrequest_data.requestId,        createdDate=skillrequest_data.createdDate,        deadlineDate=skillrequest_data.deadlineDate,        user_1_id=skillrequest_data.user_1,        skill_1_id=skillrequest_data.skill_1,        skillmatch_2_id=skillrequest_data.skillmatch_2        )
...
async def bulk_create_skillrequest(items: list[SkillRequestCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_skillrequest = SkillRequest(
                status=item_data.status.value,                requestId=item_data.requestId,                createdDate=item_data.createdDate,                deadlineDate=item_data.deadlineDate,                user_1_id=item_data.user_1,                skill_1_id=item_data.skill_1,                skillmatch_2_id=item_data.skillmatch_2            )
            database.add(db_skillrequest)
            database.flush()  # Get ID without committing
...
async def create_skill(skill_data: SkillCreate, database: Session = Depends(get_db)) -> Skill:
    ...
    db_skill = Skill(
        category=skill_data.category,        skillId=skill_data.skillId,        skillLevel=skill_data.skillLevel.value,        skillName=skill_data.skillName,        description=skill_data.description        )
...
async def bulk_create_skill(items: list[SkillCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_skill = Skill(
                category=item_data.category,                skillId=item_data.skillId,                skillLevel=item_data.skillLevel.value,                skillName=item_data.skillName,                description=item_data.description            )
            database.add(db_skill)
            database.flush()  # Get ID without committing
...
async def create_userskill(userskill_data: UserSkillCreate, database: Session = Depends(get_db)) -> UserSkill:
    ...
    db_userskill = UserSkill(
        certification=userskill_data.certification,        skillId=userskill_data.skillId,        skillLevel=userskill_data.skillLevel.value,        yearsOfExperience=userskill_data.yearsOfExperience,        user_id=userskill_data.user,        skill_id=userskill_data.skill        )
...
async def bulk_create_userskill(items: list[UserSkillCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_userskill = UserSkill(
                certification=item_data.certification,                skillId=item_data.skillId,                skillLevel=item_data.skillLevel.value,                yearsOfExperience=item_data.yearsOfExperience,                user_id=item_data.user,                skill_id=item_data.skill            )
            database.add(db_userskill)
            database.flush()  # Get ID without committing
...
def get_paginated_user(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    for user_item in user_list:
        ...
        item_data = {
            "user": user_item,
            "skillrequest_ids": [x[0] for x in skillrequest_ids],            "skillmatch_ids": [x[0] for x in skillmatch_ids],            "userskill_ids": [x[0] for x in userskill_ids],            "skillmatch_3_ids": [x[0] for x in skillmatch_3_ids]        }
        result.append(item_data)
    return {
        "total": total,
        "skip": skip,
...
async def create_user(user_data: UserCreate, database: Session = Depends(get_db)) -> User:
    ...
    db_user = User(
        userName=user_data.userName,        userId=user_data.userId,        emailId=user_data.emailId        )
...
async def bulk_create_user(items: list[UserCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_user = User(
                userName=item_data.userName,                userId=item_data.userId,                emailId=item_data.emailId            )
            database.add(db_user)
            database.flush()  # Get ID without committing
