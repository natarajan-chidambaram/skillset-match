...
async def create_skillrequest(skillrequest_data: SkillRequestCreate, database: Session = Depends(get_db)) -> SkillRequest:
    ...
    db_skillrequest = SkillRequest(
        deadlineDate=skillrequest_data.deadlineDate,        createdDate=skillrequest_data.createdDate,        status=skillrequest_data.status,        requestId=skillrequest_data.requestId,        user_1_id=skillrequest_data.user_1,        skillmatch_2_id=skillrequest_data.skillmatch_2        )
...
async def bulk_create_skillrequest(items: list[SkillRequestCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_skillrequest = SkillRequest(
                deadlineDate=item_data.deadlineDate,                createdDate=item_data.createdDate,                status=item_data.status,                requestId=item_data.requestId,                user_1_id=item_data.user_1,                skillmatch_2_id=item_data.skillmatch_2            )
            database.add(db_skillrequest)
...
async def create_skill(skill_data: SkillCreate, database: Session = Depends(get_db)) -> Skill:
    ...
    db_skill = Skill(
        skillName=skill_data.skillName,        skillId=skill_data.skillId,        category=skill_data.category,        skillLevel=skill_data.skillLevel,        description=skill_data.description        )
...
async def bulk_create_skill(items: list[SkillCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_skill = Skill(
                skillName=item_data.skillName,                skillId=item_data.skillId,                category=item_data.category,                skillLevel=item_data.skillLevel,                description=item_data.description            )
            database.add(db_skill)
            database.flush()  # Get ID without committing
...
async def create_userskill(userskill_data: UserSkillCreate, database: Session = Depends(get_db)) -> UserSkill:
    ...
    db_userskill = UserSkill(
        skillId=userskill_data.skillId,        yearsOfExperience=userskill_data.yearsOfExperience,        certification=userskill_data.certification,        skillLevel=userskill_data.skillLevel,        user_id=userskill_data.user,        skill_id=userskill_data.skill        )
...
async def bulk_create_userskill(items: list[UserSkillCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_userskill = UserSkill(
                skillId=item_data.skillId,                yearsOfExperience=item_data.yearsOfExperience,                certification=item_data.certification,                skillLevel=item_data.skillLevel,                user_id=item_data.user,                skill_id=item_data.skill            )
            database.add(db_userskill)
            database.flush()  # Get ID without committing
...
def get_paginated_user(skip: int = 0, limit: int = 100, detailed: bool = False, database: Session = Depends(get_db)) -> dict:
    ...
    for user_item in user_list:
        ...
        item_data = {
            "user": user_item,
            "skillmatch_3_ids": [x[0] for x in skillmatch_3_ids],            "skillrequest_ids": [x[0] for x in skillrequest_ids],            "userskill_ids": [x[0] for x in userskill_ids],            "skillmatch_ids": [x[0] for x in skillmatch_ids]        }
        result.append(item_data)
    return {
        "total": total,
        "skip": skip,
...
async def create_user(user_data: UserCreate, database: Session = Depends(get_db)) -> User:
    ...
    db_user = User(
        userName=user_data.userName,        emailId=user_data.emailId,        userId=user_data.userId        )
...
async def bulk_create_user(items: list[UserCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_user = User(
                userName=item_data.userName,                emailId=item_data.emailId,                userId=item_data.userId            )
            database.add(db_user)
            database.flush()  # Get ID without committing
...
async def create_session(session_data: SessionCreate, database: Session = Depends(get_db)) -> Session:
    ...
    db_session = Session(
        sessionType=session_data.sessionType,        sessionId=session_data.sessionId,        duration=session_data.duration,        sessionDate=session_data.sessionDate,        skillmatch_1_id=session_data.skillmatch_1        )
...
async def bulk_create_session(items: list[SessionCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_session = Session(
                sessionType=item_data.sessionType,                sessionId=item_data.sessionId,                duration=item_data.duration,                sessionDate=item_data.sessionDate,                skillmatch_1_id=item_data.skillmatch_1            )
            database.add(db_session)
            database.flush()  # Get ID without committing
...
async def create_review(review_data: ReviewCreate, database: Session = Depends(get_db)) -> Review:
    ...
    db_review = Review(
        comments=review_data.comments,        rating=review_data.rating,        reviewId=review_data.reviewId,        session_1_id=review_data.session_1        )
...
async def bulk_create_review(items: list[ReviewCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_review = Review(
                comments=item_data.comments,                rating=item_data.rating,                reviewId=item_data.reviewId,                session_1_id=item_data.session_1            )
            database.add(db_review)
            database.flush()  # Get ID without committing
...
async def create_skillmatch(skillmatch_data: SkillMatchCreate, database: Session = Depends(get_db)) -> SkillMatch:
    ...
    db_skillmatch = SkillMatch(
        status=skillmatch_data.status,        matchId=skillmatch_data.matchId,        startDate=skillmatch_data.startDate,        createdDate=skillmatch_data.createdDate,        user_3_id=skillmatch_data.user_3,        user_2_id=skillmatch_data.user_2        )
...
async def bulk_create_skillmatch(items: list[SkillMatchCreate], database: Session = Depends(get_db)) -> dict:
    ...
    for idx, item_data in enumerate(items):
        try:
            ...
            db_skillmatch = SkillMatch(
                status=item_data.status,                matchId=item_data.matchId,                startDate=item_data.startDate,                createdDate=item_data.createdDate,                user_3_id=item_data.user_3,                user_2_id=item_data.user_2            )
            database.add(db_skillmatch)
            database.flush()  # Get ID without committing
